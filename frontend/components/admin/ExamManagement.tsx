...
import { upcomingExams, liveExams, completedExams, cancelledExams } from "@/lib/mock-data";

type Exam = {
  id: string; name: string; subject: string; duration: number;
  students: number; status: string; date: string;
};
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-semibold text-foreground truncate group-hover:text-primary transition-colors">{exam.name}</h4>
          <p className="text-xs text-muted-foreground mt-0.5">{exam.subject}</p>
        </div>
        <span className={`flex-shrink-0 flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border ${cfg.color}`}>
      ...
      <div className="flex items-center gap-4 text-xs text-muted-foreground">
        ...
        <span className="flex items-center gap-1"><Users className="w-3 h-3" />{exam.students}</span>
        <span className="flex items-center gap-1"><BookMarked className="w-3 h-3" />{exam.subject}</span>
      </div>
